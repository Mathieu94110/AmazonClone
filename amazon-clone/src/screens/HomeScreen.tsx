import { Text, ScrollView, StyleSheet, TextInput, View, Image } from "react-native";
import React, { useEffect, useState } from "react";
import DeliveryAddressCard from "../components/DeliveryAddressCard";
import CategoryCard from "../components/CategoryCard";
import CarouselCard from "../components/CarouselCard";
import { categoryData, devicesDealData, dealData } from "../data/CarouselData";
import MaterialIcons from "react-native-vector-icons/MaterialIcons";
import DealCard from "../components/DealCard";

const HomeScreen = ({ navigation, route }) => {
  const [userGivenName, setUserGivenName] = useState<string>("");
  useEffect(
    () =>
      navigation.setOptions({
        headerTitle: "",
        header: () => (
          <View style={styles.homeScreenHeader}>
            <View style={styles.homeScreenHeaderAmazonImgContainer}>
              <Image
                style={styles.homeScreenHeaderAmazonImg}
                source={{
                  uri: "data:image/png;base64,",
                }}
              />
            </View>
            <View style={styles.homeScreenHeaderTextInputContainer}>
              <MaterialIcons name="search" color={"gray"} size={20} />
              <TextInput
                style={styles.homeScreenHeaderTextInput}
                placeholder="Rechercher sur Amazon.fr"
                placeholderTextColor="gray"
              />
            </View>
            <Text style={styles.homeScreenHeaderUserGivenName}>Bonjour {userGivenName}</Text>
            <View style={styles.homeScreenHeaderMicIconContainer}>
              <MaterialIcons name="mic-none" size={26} color={"#fff"} />
            </View>

            <View></View>
          </View>
        ),
      }),
    [],
  );

  useEffect(() => {
    setUserGivenName(route.params.given_name);
  }, []);

  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      <DeliveryAddressCard />
      <ScrollView
        style={styles.homeScreenCategoryContainer}
        horizontal={true}
        showsHorizontalScrollIndicator={false}
      >
        {categoryData.map((data, index) => (
          <CategoryCard key={index} img={data.img} text={data.text} />
        ))}
      </ScrollView>

      <CarouselCard />
      <View>
        <Text style={styles.homeScreenDealText}>Appareils électroniques pour la maison</Text>
        <View style={styles.homeScreenDealItemCont}>
          {devicesDealData.map((data, index) => (
            <DealCard key={index} img={data.img} text={data.text} />
          ))}
        </View>
      </View>
      <View>
        <Text style={styles.homeScreenDealText}> Plus de 60% de remise | équipement ménager</Text>
        <View style={styles.homeScreenDealItemCont}>
          {dealData.map((data, index) => (
            <DealCard key={index} img={data.img} text={data.text} />
          ))}
        </View>
      </View>
    </ScrollView>
  );
};

export default HomeScreen;

const styles = StyleSheet.create({
  homeScreenHeader: {
    backgroundColor: "#232f3e",
    flexDirection: "row",
    width: "100%",
    height: 70,
  },
  homeScreenHeaderAmazonImgContainer: {
    flexDirection: "row",
    marginVertical: 15,
    marginHorizontal: 2,
  },
  homeScreenHeaderAmazonImg: {
    width: 70,
    height: "auto",
  },
  homeScreenHeaderTextInputContainer: {
    flexDirection: "row",
    width: "50%",
    alignItems: "center",
    borderWidth: 1,
    borderColor: "#b8baba",
    marginLeft: 0,
    margin: 15,
    backgroundColor: "white",
    borderRadius: 5,
    overflow: "hidden",
    height: 40,
    paddingLeft: 3,
  },
  homeScreenHeaderTextInput: {
    width: "92%",
    height: "90%",
    borderWidth: 0,
    paddingLeft: 4,
    backgroundColor: "white",
    paddingBottom: 2,
  },
  homeScreenHeaderUserGivenName: {
    flexDirection: "row",
    height: "100%",
    justifyContent: "center",
    width: "18%",
    fontWeight: "600",
    color: "#fff",
    fontSize: 12,
    marginTop: 20,
    textAlign: "center",
  },
  homeScreenHeaderMicIconContainer: {
    flexDirection: "row",
    alignItems: "center",
    height: "100%",
    justifyContent: "center",
    width: "8%",
  },
  homeScreenCategoryContainer: {
    flex: 1,
    paddingLeft: 15,
    paddingTop: 10,
    paddingBottom: 10,
  },
  homeScreenDealText: {
    fontSize: 16,
    fontWeight: "bold",
    marginBottom: 20,
    marginLeft: 10,
    marginTop: 20,
  },
  homeScreenDealItemCont: {
    flexDirection: "row",
    justifyContent: "space-around",
    flexWrap: "wrap",
  },
});
